...
public class GroupServiceImpl implements GroupService {
    ...
    public Result groupInvite(@NonNull GroupInviteDto dto) {
        long startTime = System.currentTimeMillis();
        try {
            String groupId = StringUtils.hasText(dto.getGroupId()) ? dto.getGroupId()
            ...
            try {
                ...
                ImGroupPo groupPo = cachedGroup != null ? (ImGroupPo) cachedGroup
    ...
    private Result processDirectJoin(String groupId, String userId, String inviterId, long startTime) {
        ...
        try {
            ...
            if (success) {
                // 异步更新群信息和发送通知
                CompletableFuture.runAsync(() -> updateGroupInfoAndNotify(groupId, inviterId, userId),
                        asyncTaskExecutor);
            }
    ...
    private void sendJoinApprovalRequestToAdmins(String groupId, String inviterId, String inviteeId,
            ImGroupPo groupPo) {
        try {
            // 直接从数据库获取成员列表，不再使用缓存
            List<ImGroupMemberPo> members = imGroupMemberDubboService.selectList(groupId);
